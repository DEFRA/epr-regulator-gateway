...
public sealed class ClientCredentialsAccessTokenProvider : IClientCredentialsAccessTokenProvider, IDisposable
{
    ...
    public ClientCredentialsAccessTokenProvider(
        IOptions<ClientCredentialsTokenOptions> options,
    ...
    public async Task<string> GetAccessTokenAsync(CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(_options.Scope))
        {
            throw new InvalidOperationException(
                "Client credentials token acquisition requires a non-empty Scope in ClientCredentialsTokenOptions.");
        }
